...
def sm100_gemm_f16_tma(A, B, C=None, alpha=1.0, beta=0.0, check_alignment=True):
    ...
    return sm100_gemm.gemm_f16(A, B, C, alpha, beta)


# Keep the old name for compatibility
sm100_gemm_f16 = sm100_gemm.gemm_f16
...
def create_aligned_tensors(
    ...
    """
    aligned_M, aligned_N, aligned_K = (M, N, K)
...
def benchmark_sm100_vs_torch(
    ...
    # warmup
    sm100_result = gemm_f16(A, B, C.clone(), check_alignment=False)
...
def stress_test_large_matrices():
    ...
    for M, N, K in test_sizes:
        try:
            ...
            # Warmup
            for _ in range(3):
                result = gemm_f16(A, B, C.clone(), check_alignment=False)
