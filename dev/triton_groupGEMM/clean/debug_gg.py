...
from tgrouped_gemm_forward import _grouped_gemm, grouped_gemm_forward
...
def debug_grouped_gemm():
    ...
    try:
        ...
        from tgrouped_gemm_forward import _grouped_gemm

        logging.info(
            f"Forward function signature: {getsource(_grouped_gemm).splitlines()[0]}"
        )
    ...
    try:
        ...
        if len(pruned_configs) == 0:
            ...
            logging.info(
                f"Is minimal config valid? {req_shared_mem <= max_shared_memory}"
            )
        ...
        # Forward pass
        result = grouped_gemm_forward(x_back, w_back, m_sizes)
        ...
        try:
            ...
            try:
                grad_x, grad_w = grouped_gemm_backward(
                    grad_output, x_back, w_back, m_sizes
                )
